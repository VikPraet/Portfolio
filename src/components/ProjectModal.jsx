  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto">
        ...
        >
          {/* Outer wrapper that holds the border effect */}
          <div
          ...
          >
            ...
            <div
              className={`relative bg-n-7 text-n-1 rounded-lg shadow-lg p-8 w-full max-w-5xl mx-auto z-10 modal-content
                ${isVisible ? 'modal-enter-active' : 'modal-enter'} 
                ${isClosing ? 'modal-exit-active' : ''}`}
              onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside the modal
              style={{ minHeight: '300px' }} // Ensure the modal doesn't shrink if empty
            >
              {/* Close button */}
              <button
                ...
                {project.content &&
                  project.content.map((block, index) => {
                    switch (block.type) {
                      case 'text':
                        return (
                          <p key={index} className="text-lg text-gray-300 break-words">
                            {block.value}
                          </p>
                        );
                      case 'image':
